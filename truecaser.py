...
Model = namedtuple('Model', ('uni_dist',
                                 'backward_bi_dist',
                                 'forward_bi_dist',
                                 'trigram_dist',
                                 'word_casing_lookup'))
...
def get_score(prev_token, pos_token, next_token, model):
    ...
    if prev_token is not None:
        ...
        denominator = 0
        for alt_token in model.word_casing_lookup[pos_token.lower()]:
            denominator += model.backward_bi_dist[prev_token + '_' + alt_token] + pseudo_count
    ...
    if next_token is not None:
        ...
        denominator = 0
        for alt_token in model.word_casing_lookup[pos_token.lower()]:
            denominator += model.forward_bi_dist[alt_token + "_" + next_token] + pseudo_count
    ...
    if prev_token is not None and next_token is not None:
        next_token = next_token.lower() #Ensure it is lower case
        nominator = model.trigram_dist[prev_token + "_"
                                       + pos_token + "_"
                                       + next_token] + pseudo_count
        denominator = 0
        for alt_token in model.word_casing_lookup[pos_token.lower()]:
            denominator += model.trigram_dist[prev_token + "_"
                                              + alt_token + "_"
                                              + next_token] + pseudo_count

        trigram_score = nominator / denominator

    result = (math.log(unigram_score)
...
def get_true_case(tokens, out_of_vocab_token_opt, model):
    ...
    for token_idx, token in enumerate(tokens):
        if token in string.punctuation or token.isdigit():
            ...
            if token in model.word_casing_lookup:
                if len(model.word_casing_lookup[token]) == 1:
                    tokens_true_case.append(list(model.word_casing_lookup[token])[0])
                else:
                    ...
                    for pos_token in model.word_casing_lookup[token]:
